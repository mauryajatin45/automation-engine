...
async function createMainBackup() {
  console.log('🚀 Extracting current restored catalogue to create Main Backup file...');
  
  let hasNextPage = true;
  let cursor = null;
  let totalBackedUp = 0;
  
  // Use a clear, distinguishable name for this main master backup
  const backupFile = 'main_clean_backup_catalog.json';
  ...
  try {
    while (hasNextPage) {
      ...
      console.log(`[Progress] Saved ${totalBackedUp} products...`);
    }
    
    fs.appendFileSync(backupFile, '\n]\n');
    console.log(`\n========================================================================`);
    console.log(`✅ Main Backup Complete!`);
    console.log(`Successfully backed up ${totalBackedUp} products to: ${backupFile}`);
    console.log(`========================================================================\n`);
