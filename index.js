...
app.post('/api/test-rich-descriptions-2', async (req, res) => {
  try {
    const GET_COLLECTIONS = `
      query {
        collections(first: 5, query: "handle:differentials-lockers OR handle:uhf-radios OR handle:throttle-controllers OR handle:camping-equipment OR handle:recovery-gear") {
          edges {
            node {
              id
              title
    ...
    console.log("🚀 Fetching different collections and product listings from Shopify...");
