...
const makers = [
  ...
  "ISUZU", "NISSAN", "MITSUBISHI", "MERCEDES", "BMW", 
  "AUDI", "CHRYSLER", "VOLKSWAGEN"
];

async function createHubs() {
  console.log('🚀 Creating Base Maker Hub Collections...');

  for (const maker of makers) {
    const input = {
      title: maker,
      handle: maker.toLowerCase().replace(/\s+/g, '-'),
      descriptionHtml: `<p>Shop premium aftermarket parts and accessories for your ${maker}.</p>`,
    };
    ...
    try {
      ...
      if (response.data.collectionCreate.userErrors.length > 0) {
        console.error(`❌ Error creating ${maker}:`, response.data.collectionCreate.userErrors[0].message);
      } else {
      ...
      console.error(`Fatal Error on ${maker}:`, error.message);
    }
  }
}
